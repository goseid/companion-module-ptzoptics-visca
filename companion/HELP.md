...
Several presets — the camera preset buttons, **Absolute Position**, **Focus Direct**, **Gain / Bright / Exp Comp Set**, and **RB Gain Direct** — are driven by **button-local variables**. To make a custom button, drag one in, then copy it and edit its local variable(s) (for example `PresetNumber`, or `PanPosition` / `TiltPosition`) in the button's **Local Variables** section. The button's text, actions, and feedback all follow the variable, so you only change the value in one place instead of editing every field. The camera preset buttons also carry a `PresetName` variable (default "Preset") that sets the top line of the button text — set it to a friendly label (e.g. "Wide Stage") without changing what preset the button recalls or saves.
